...
int main()
{
	...
	c.data[2] = 4;
	c.client_pid = getpid();
	printf("<---------------Program Client2 Started----------------->\n");
//	printf("CLIENT2: Data to be Written to Server : Sign = %d, Val1 = %d, Val2 = %d, C1PID: = %d\n",c.data[0],c.data[1],c.data[2],c.client_pid);	
...
//	printf("\nCLIENT2: %d Bytes Written to Server\n",ret);
	//sleep(1);
	...
	printf("_____\t\tCLIENT2: 5 - 4 = %d______\n",buff[0]);
	close(client_fd);
	close(server_fd);
	unlink(client_fifo);
	printf("<---------------Program Client2 Ended--------------------->\n");
	exit(0);
}
