...
//        sprintf(arr1,"%d%c%d",arr[1],arr[0],arr[2]);
        ...
        if(access("fifo_client2",F_OK==-1))
        file1=mkfifo("fifo_client2",0777);
        if(file1<0)
        {
        ...
        fd3=open("fifo_client2",O_RDONLY);
        printf("file discriptor is %d\n",fd3);
     //   sleep(2);
        recived=read(fd3,&datavar1,sizeof(datavar1));
