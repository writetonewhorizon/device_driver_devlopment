...
ssize_t dev_write(struct file * filep, const char __user *ubuff, size_t count, loff_t * pos)
{
	...
	ret=copy_from_user(ch,ubuff,count);
	if(ret==0)
	{
		#ifdef DEFINE
		printk(KERN_INFO " data written is %c",ch[0]);
		#endif
	}
	else 
	{
...
/////////////////////////////////////////////>>>>>>>>>>>>>>>>>>>>>>>>>>>to set baud rate<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<</////////////////////////
		outb(128,BASE+3); //to DLAB enable
		mdelay(100);
		outb(12,BASE+0)
		mdelay(100);
		outb(0,BASE+1);
		mdelay(100);
		outb(27,BASE+3); // to reset
		mdelay(100);
/////////////////////////////////////////////>>>>>>>>>>>>>>>>>>>>>>>>>>>to set baud rate<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<</////////////////////////
/////////////////////////////////////////////>>>>>>>>>>>>>>>>>>>>>>>>>>>to enable fifo<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<</////////////////////////
		outb(13,BASE+2);
